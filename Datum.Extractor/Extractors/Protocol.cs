...
public sealed class Protocol(JsonNode node) : IExtractor<Protocol>
{
    ...
    public Server Server => server ??= new Server(node.AsObject());

    private Server? server;

    public Client Client => client ??= new Client(node.AsObject());
...
public sealed class Server(JsonObject node)
{
...
public sealed class Client(JsonObject node)
{
